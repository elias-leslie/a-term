...
import { Folder, Search, Terminal, X } from 'lucide-react'
...
export interface AttachableTerminalOption {
  id: string
  label: string
  description: string
...
export interface ProjectRowData {
  ...
  paneCount: number
  attachableOptions: AttachableTerminalOption[]
}
...
export function formatDetachedPaneDescription(pane: TerminalPane): string {
...
export function makeExternalAttachableOption(session: TerminalSession): AttachableTerminalOption {
  return {
    id: session.id,
...
export function makeDetachedPaneAttachableOption(pane: TerminalPane): AttachableTerminalOption {
  return {
    id: pane.id,
...
export function makeProjectSessionOptions(
  ...
  detachedPanes: TerminalPane[],
): AttachableTerminalOption[] {
  return [
...
export function filterAndSortSessions(
  sessions: AttachableTerminalOption[],
  query: string,
): AttachableTerminalOption[] {
...
interface ProjectSessionRowProps {
  ...
  onAttachSession: (option: AttachableTerminalOption) => void
...
interface SessionSectionProps {
  ...
  countLabel: string
  total: number
  visible: AttachableTerminalOption[]
  searchQuery: string
  emptyLabel: string
  actionLabel: string
  onAction: (session: AttachableTerminalOption) => void
}
...
export function buildProjectRows(
  ...
  const externalSessionMap = new Map<string, TerminalSession[]>()
  const detachedPaneMap = new Map<string, TerminalPane[]>()
    ...
    <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: '1px solid var(--term-border)' }}>
      <div className="min-w-0">
        ...
        <Dialog.Description className="mt-0.5 text-xs" style={{ color: 'var(--term-text-muted)' }}>
          Create, attach, or manage terminal sessions across projects.
        </Dialog.Description>
      </div>
  ...
  return (
    <>
      ...
      >
        ...
        <span className="min-w-0 flex-1">
          <span className="block text-sm font-medium truncate">New Ad-Hoc Terminal</span>
          <span className="block text-[11px] truncate" style={{ color: 'var(--term-text-muted)', fontFamily: 'var(--font-mono)' }}>
          ...
          <div className="space-y-1.5 pr-1 md:max-h-[420px] md:overflow-y-auto">
            {visibleProjectRows.map((row) => (
              <ProjectSessionRow
                ...
                onCreateProjectTerminal={onCreateProjectTerminal}
              />
            ))}
          </div>
        )}
  ...
  return (
    <div className="mt-6 text-center py-8 rounded-lg" style={{ backgroundColor: 'var(--term-bg-surface)', border: '1px solid var(--term-border)' }}>
      ...
      <p className="text-xs mt-1" style={{ color: 'var(--term-text-muted)', opacity: 0.6 }}>
        Try a different search term or create a new terminal.
      </p>
    </div>
  )
}
