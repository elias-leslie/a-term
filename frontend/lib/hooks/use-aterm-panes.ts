...
import * as api from './aterm-panes-api'
import type {
  PaneListResponse,
  UpdatePaneRequest,
} from './aterm-panes-types'

const invalidatePanesAndSessions = (qc: ReturnType<typeof useQueryClient>) => {
  qc.invalidateQueries({ queryKey: ['aterm-panes'] })
  qc.invalidateQueries({ queryKey: ['aterm-detached-panes'] })
  qc.invalidateQueries({ queryKey: ['aterm-sessions'] })
}
...
export function useATermPanes() {
  ...
  const updateMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ['aterm-panes'] }),
  })
  ...
  const swapMutation = useMutation({
    ...
    onMutate: async ({ pane_id_a, pane_id_b }) => {
      await queryClient.cancelQueries({ queryKey: ['aterm-panes'] })
      const previous = queryClient.getQueryData<PaneListResponse>(['aterm-panes'])
      ...
      if (previous) {
        ...
        if (paneA && paneB) {
          ...
          queryClient.setQueryData(['aterm-panes'], { ...previous, items })
        }
      }
      return { previous }
    },
    onError: (_e, _v, ctx) => ctx?.previous && queryClient.setQueryData(['aterm-panes'], ctx.previous),
    onSettled: () => queryClient.invalidateQueries({ queryKey: ['aterm-panes'] }),
  })
