...
import { A_TERM_THEMES, type ATermThemeId } from '../constants/a-term'

// Re-export theme type and themes for convenience
export { A_TERM_THEMES, type ATermThemeId }

// Popular monospace fonts for A-Term sessions
// Mix of Google Fonts (loaded) and system fonts (fallback)
export const A_TERM_FONTS = [
  {
    id: 'jetbrains-mono',
    name: 'JetBrains Mono',
...
export const A_TERM_FONT_SIZES = [10, 11, 12, 13, 14, 15, 16, 18, 20] as const

// Scrollback buffer sizes (lines) - capped at 50K to avoid progressive slowdown
export const A_TERM_SCROLLBACK_OPTIONS = [
...
export const A_TERM_CURSOR_STYLES = ['block', 'underline', 'bar'] as const

export type ATermFontId = (typeof A_TERM_FONTS)[number]['id']
export type ATermFontSize = (typeof A_TERM_FONT_SIZES)[number]
export type ATermScrollback =
  (typeof A_TERM_SCROLLBACK_OPTIONS)[number]['value']
export type ATermCursorStyle = (typeof A_TERM_CURSOR_STYLES)[number]
...
const STORAGE_KEY_GLOBAL = 'a-term-settings'
const STORAGE_KEY_PROJECT_PREFIX = 'a-term-settings-project-'
...
function parseSettings(parsed: Record<string, unknown>): ATermSettings {
  return {
    fontId: A_TERM_FONTS.some((f) => f.id === parsed.fontId)
      ? (parsed.fontId as ATermFontId)
      : DEFAULT_SETTINGS.fontId,
    fontSize: A_TERM_FONT_SIZES.includes(parsed.fontSize as ATermFontSize)
      ? (parsed.fontSize as ATermFontSize)
      : DEFAULT_SETTINGS.fontSize,
    scrollback: A_TERM_SCROLLBACK_OPTIONS.some(
      (o) => o.value === parsed.scrollback,
    )
      ? (parsed.scrollback as ATermScrollback)
      : DEFAULT_SETTINGS.scrollback,
    cursorStyle: A_TERM_CURSOR_STYLES.includes(
    ...
    themeId:
      parsed.themeId && (parsed.themeId as string) in A_TERM_THEMES
...
export function useATermSettings(projectId?: string) {
  ...
  const fontFamily =
    A_TERM_FONTS.find((f) => f.id === settings.fontId)?.family ??
    A_TERM_FONTS[0].family
  ...
  const theme = A_TERM_THEMES[settings.themeId].theme
