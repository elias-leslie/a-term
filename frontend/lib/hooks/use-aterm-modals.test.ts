...
describe('useATermModals', () => {
  ...
  it('opens the aterm manager when the URL modal param targets it', () => {
    mockSearchParams.set('modal', 'aterm-manager')
  ...
  it('runs the external attach callback while keeping session URL sync', () => {
    ...
    result.current.handleAttachExternalSession('codex-aterm')

    expect(onAttachExternalSession).toHaveBeenCalledWith('codex-aterm')
    expect(mockReplace).toHaveBeenCalledWith('/?session=codex-aterm', { scroll: false })
  })
