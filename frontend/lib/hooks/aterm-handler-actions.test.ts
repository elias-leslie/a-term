...
import { addProjectPaneAction } from './aterm-handler-actions'
import { waitForTmuxInit } from './aterm-handler-utils'

vi.mock('./aterm-handler-utils', async () => {
  const actual = await vi.importActual<typeof import('./aterm-handler-utils')>(
    './aterm-handler-utils',
  )
  return {
    ...actual,
