...
import type {
  DeleteSessionResult,
  ATermSession,
} from '@/lib/hooks/use-aterm-sessions'
import {
  getSlotSessionId,
  isPaneSlot,
  type PaneSlot,
...
export function useATermSlotHandlers({
  ...
  // Handler for resetting a slot's aterm
  ...
  const handleSlotReset = useCallback(
    async (slot: ATermSlot) => {
      ...
      await queryClient.invalidateQueries({ queryKey: ['aterm-panes'] })
  ...
  // Handler for closing a slot's aterm
