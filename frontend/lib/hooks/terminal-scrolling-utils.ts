...
export function isAlternateScreen(terminal: Terminal): boolean {
  return terminal.buffer.active.type === 'alternate'
}

export function isMouseTrackingActive(terminal: Terminal): boolean {
...
export function refreshTerminalViewport(terminal: Terminal): void {
  ...
  // xterm's public terminal.refresh() can leave touch-driven scrollLines()
...
export function setupTouchHandlers(
  ...
  const handleTouchStart = (e: TouchEvent) => {
    ...
    if (
      terminal &&
      !isAlternateScreen(terminal) &&
      isMouseTrackingActive(terminal)
    ) {
      deps.enterCopyMode()
    }
  }
  ...
  const handleTouchMove = (e: TouchEvent) => {
    ...
    // matching the rest of mobile terminal scrolling.
    ...
    if (isMouseTrackingActive(terminal)) {
      e.preventDefault()
      e.stopPropagation()
      const deltaY = lastSentY - currentY
      if (Math.abs(deltaY) >= SCROLL_THRESHOLD) {
