...
export interface UseTerminalDiagnosticsReturn {
  enabled: boolean
...
function isEnabled(): boolean {
  ...
  if (new URLSearchParams(window.location.search).get('diagnostics') === '1')
    return true
  try {
    return localStorage.getItem('terminal:diagnostics') === '1'
  } catch {
    return false
  }
}

export function useTerminalDiagnostics(
  sessionId: string | undefined,
): UseTerminalDiagnosticsReturn {
  ...
  useEffect(() => {
    ...
    const target = window as typeof window & {
      __terminalDiagnostics?: Record<
        string,
        ReturnType<typeof getDiagnostics>
      >
    }
    if (!target.__terminalDiagnostics) {
      target.__terminalDiagnostics = {}
    }
    target.__terminalDiagnostics[sessionId] = getDiagnostics()
    return () => {
      if (target.__terminalDiagnostics) {
        delete target.__terminalDiagnostics[sessionId]
      }
    }
  ...
  useEffect(() => {
    ...
    const interval = window.setInterval(() => {
      if (!target.__terminalDiagnostics) return
      target.__terminalDiagnostics[sessionId] = getDiagnostics()
    }, 250)
