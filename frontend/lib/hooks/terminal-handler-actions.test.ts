...
import { addProjectPaneAction } from './terminal-handler-actions'
import { waitForTmuxInit } from './terminal-handler-utils'

vi.mock('./terminal-handler-utils', async () => {
  const actual = await vi.importActual<typeof import('./terminal-handler-utils')>(
    './terminal-handler-utils',
  )
  return {
    ...actual,
