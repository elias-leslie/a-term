...
import { getDefaultLayoutMode, type LayoutMode } from '@/lib/constants/terminal'
...
export function useTerminalTabsState({
  ...
  const orderedIds = useMemo(
    () => getOrderedIds(terminalSlots),
    [terminalSlots],
  )
  const reorder = useCallback((_newOrder: string[]) => {
    /* noop — reorder via drag-and-drop not yet implemented */
  }, [])
  ...
  const { activeStatus, showReconnect } = useConnectionStatus(
    activeSessionId,
    terminalStatuses,
  )
