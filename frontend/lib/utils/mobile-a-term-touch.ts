...
export const MOBILE_A_TERM_TOUCH_SURFACE_SELECTORS = [
  '.xterm-screen',
  '.xterm-viewport',
] as const
...
/**
 * Apply touch-safe scrolling styles to the a-term container and the xterm
...
export function applyMobileATermTouchStyles(container: HTMLElement): void {
  ...
  for (const selector of MOBILE_A_TERM_TOUCH_SURFACE_SELECTORS) {
    ...
    if (selector === '.xterm-viewport') {
      element.classList.add(MOBILE_A_TERM_VIEWPORT_CLASS)
