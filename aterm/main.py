...
Independent microservice for web aterm functionality.
...
from .api import agent, agent_tools, aterm, diagnostics, files, panes, projects, sessions
from .branding import DESCRIPTION, DISPLAY_NAME, get_cache_root
from .config import ATERM_PORT, CORS_ORIGINS
...
def _setup_tmux_options(token: str) -> None:
    """Set up tmux options and hooks for aterm service.
    ...
    to avoid affecting non-web-aterm sessions (e.g., MobaXterm, other clients).
    ...
    hook_cmd = (
        ...
        f" 'http://localhost:{ATERM_PORT}/api/internal/session-switch"
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("aterm_service_starting", port=ATERM_PORT)
    ...
    # Shutdown
    logger.info("aterm_service_stopping")
    await stop_scheduler(app)
    await close_client()
    close_pool()
    logger.info("aterm_service_shutdown_complete")
...
# Include routers
app.include_router(aterm.router)
...
async def health() -> dict[str, object] | JSONResponse:
    ...
    checks: dict[str, object] = {"service": "aterm"}
