...
def _resurrect_session_record(
    ...
    aterm_store.update_session(session_id, name=name, working_dir=working_dir, is_alive=True)
    try:
        ...
        aterm_store.mark_dead(session_id)
        raise
...
def _claim_and_resurrect(
    ...
    claimed = aterm_store.claim_dead_session_by_project(project_id, mode)
    if not claimed:
        return None
...
def _try_resurrect_tmux(session_id: str, working_dir: str | None) -> bool:
    ...
    try:
        create_tmux_session(session_id, working_dir)
        aterm_store.update_session(session_id, is_alive=True)
...
def create_session(
    ...
    """Create a new aterm session atomically (DB + tmux, rollback on failure).
    ...
    session_id = aterm_store.create_session(
    ...
    try:
        ...
        aterm_store.delete_session(session_id)
        raise
...
def delete_session(session_id: str) -> bool:
    """Delete a aterm session atomically. Idempotent."""
    kill_tmux_session(session_id, ignore_missing=True)
    deleted = aterm_store.delete_session(session_id)
    if deleted:
...
def reset_project_sessions(
    ...
    all_sessions = aterm_store.get_all_project_sessions(project_id)
...
def reset_all_sessions() -> int:
    """Reset all aterm sessions globally. Returns count reset."""
    sessions = aterm_store.list_sessions()
    count = sum(1 for session in sessions if reset_session(session["id"]))
    logger.info("all_sessions_reset", count=count)
    return count


def disable_project_aterm(project_id: str) -> bool:
    """Delete all project sessions and mark the project aterm as disabled."""
    all_sessions = aterm_store.get_all_project_sessions(project_id)
    for session in all_sessions:
        delete_session(session["id"])
    settings_store.upsert_settings(project_id, enabled=False)
    logger.info("project_aterm_disabled", project_id=project_id, deleted_sessions=len(all_sessions))
    return True
...
def reconcile_sessions(purge_after_days: int = 7) -> dict[str, int]:
    ...
    db_sessions = aterm_store.list_sessions(include_dead=True)
    tmux_sessions = list_tmux_sessions()
    ...
    purged = aterm_store.purge_dead_sessions(
    ...
    remaining_ids = {s["id"] for s in aterm_store.list_sessions(include_dead=True)}
