"""A-Term Projects API - Project settings for aterm tabs.

This module provides:
- List projects with aterm settings merged
- Update aterm settings per project
...
class ProjectResponse(BaseModel):
    """Project with aterm settings merged."""

    id: str
    name: str
    root_path: str | None
    # A-Term-specific settings
    aterm_enabled: bool = False
    mode: str = "shell"  # Active mode (shell or agent tool slug)
    display_order: int = 0


class ProjectSettingsUpdate(BaseModel):
    """Request to update aterm settings for a project."""
...
def _build_project_response(
    ...
    """Merge SummitFlow project metadata with local aterm settings."""
    project = project_lookup.get(project_id, {})
    return ProjectResponse(
        id=project_id,
        name=project.get("name", project_id),
        root_path=project.get("root_path"),
        aterm_enabled=settings["enabled"] if settings else False,
...
@router.get("/api/aterm/projects", response_model=list[ProjectResponse])
async def list_projects() -> list[ProjectResponse]:
    """List all projects with aterm settings merged.

    Fetches projects from SummitFlow API and merges with local
    aterm_project_settings. Projects without settings get defaults.
    """
    project_lookup = await _get_project_lookup()

    # Get all local aterm settings
...
@router.put("/api/aterm/project-settings/{project_id}", response_model=ProjectResponse)
async def update_project_settings(
    project_id: str,
    update: ProjectSettingsUpdate,
) -> ProjectResponse:
    """Update aterm settings for a project.
...
@router.post("/api/aterm/project-settings/bulk-order", response_model=list[ProjectResponse])
...
@router.post("/api/aterm/projects/{project_id}/reset")
@limiter.limit("5/minute")
async def reset_project(request: Request, project_id: str) -> dict[str, Any]:
    """Reset all aterm sessions for a project.
...
@router.post("/api/aterm/projects/{project_id}/disable", response_model=ProjectResponse)
async def disable_project(project_id: str) -> ProjectResponse:
    """Disable aterm for a project.

    Deletes all sessions and sets enabled=false in settings.
    """
    lifecycle.disable_project_aterm(project_id)
    project_lookup = await _get_project_lookup()
    settings = settings_store.get_all_settings().get(project_id)
    return _build_project_response(project_id, settings, project_lookup)


@router.put("/api/aterm/projects/{project_id}/mode", response_model=ProjectResponse)
