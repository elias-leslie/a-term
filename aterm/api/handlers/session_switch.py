...
def _track_session_switch(aterm_session_id: str, to_session: str) -> dict[str, Any]:
    """Update stored session target for the given aterm session.
    ...
    if to_session.startswith(BASE_SESSION_PREFIX):
        logger.info(LOG_SWITCH_TO_BASE, aterm=aterm_session_id)
        aterm_store.update_claude_session(aterm_session_id, None)
        return {"status": STATUS_CLEARED}

    logger.info(LOG_SWITCH_DETECTED, aterm=aterm_session_id, target=to_session)
    aterm_store.update_claude_session(aterm_session_id, to_session)
...
def _extract_aterm_session_id(
    from_session: str,
) -> tuple[str, JSONResponse | None]:
    """Extract and validate the aterm session UUID from the base session name.
    ...
    """
    aterm_session_id = from_session[len(BASE_SESSION_PREFIX):]
    try:
        uuid.UUID(aterm_session_id)
    except ValueError:
        logger.warning(LOG_INVALID_SESSION_ID, raw=aterm_session_id)
        return "", JSONResponse(
            status_code=400,
            content={"error": ERROR_INVALID_SESSION_ID},
        )
    return aterm_session_id, None
...
def handle_session_switch(
    ...
    # Only track switches FROM a aterm base session
    ...
    aterm_session_id, error = _extract_aterm_session_id(from_session)
    if error is not None:
        return error

    return _track_session_switch(aterm_session_id, to_session)
