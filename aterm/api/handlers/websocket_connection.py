...
async def _wait_for_initial_resize(
    ...
    """Wait for initial resize event from frontend (syncs aterm dimensions)."""
    try:
...
def _make_output_flush_callback(
    ...
    async def on_flush(batch: str) -> None:
        aterm_metrics.inc("flush_count")  # type: ignore[union-attr]
        aterm_metrics.inc("total_bytes_flushed", len(batch.encode("utf-8")))  # type: ignore[union-attr]
        aterm_metrics.inc("messages_sent")  # type: ignore[union-attr]
...
async def _teardown_session_resources(
    ...
    aterm_metrics.dec("active_connections")  # type: ignore[union-attr]
    aterm_metrics.dec("active_sessions")  # type: ignore[union-attr]
...
async def _run_message_loop(
    ...
    try:
        ...
        logger.info("aterm_disconnected", session_id=session_id)
    finally:
...
async def _run_session(
    ...
    """Set up and run the full aterm session. Returns (pid, master_fd) for cleanup."""
    capabilities: list[str] = []
    try:
    ...
    aterm_metrics = get_metrics()
    aterm_metrics.inc("active_connections")
    aterm_metrics.inc("active_sessions")
    aterm_metrics.inc("total_sessions_created")
    ...
    on_flush = _make_output_flush_callback(aterm_metrics, scrollback_tracker, recorder)
    ...
    try:
        ...
        await _teardown_session_resources(
            ...
            scrollback_sync, recorder, diag_registry, aterm_metrics,
        )
    return pid, master_fd


async def handle_aterm_connection(
    websocket: WebSocket,
    session_id: str,
) -> None:
    """Handle a aterm WebSocket connection.

    Protocol: text input, binary resize (JSON {cols, rows}), text output.
    """
    await websocket.accept()
    logger.info("aterm_connected", session_id=session_id)
    pid: int | None = None
    master_fd: int | None = None
    try:
        pid, master_fd = await _run_session(websocket, session_id)
    except Exception as e:
        logger.error("aterm_error", session_id=session_id, error=str(e))
        with contextlib.suppress(Exception):
            await websocket.close(code=1011, reason="Internal server error")
    finally:
        if pid is not None and master_fd is not None:
            await _cleanup_pty_process(pid, master_fd)
        logger.info("aterm_cleanup_complete", session_id=session_id)
