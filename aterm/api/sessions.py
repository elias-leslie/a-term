...
class ATermSessionListResponse(BaseModel):
    """Response for listing aterm sessions."""
...
class UpdateSessionRequest(BaseModel):
    """Request to update a aterm session."""
...
@router.get("/api/aterm/sessions", response_model=ATermSessionListResponse)
async def list_sessions() -> ATermSessionListResponse:
    """List all alive aterm sessions.

    Returns only sessions where is_alive=True.
    Sessions are ordered by display_order, then created_at.
    """
    sessions = aterm_store.list_sessions(include_dead=False, include_detached=False)
...
@router.get("/api/aterm/sessions/{session_id}", response_model=ATermSessionResponse)
async def get_session(session_id: str) -> ATermSessionResponse:
    """Get a single aterm session by ID."""
...
@router.patch("/api/aterm/sessions/{session_id}", response_model=ATermSessionResponse)
async def update_session(session_id: str, request: UpdateSessionRequest) -> ATermSessionResponse:
    """Update aterm session metadata.
    ...
    existing = aterm_store.get_session(session_id)
    if not existing:
        raise HTTPException(status_code=404, detail=f"Session {session_id} not found") from None
    ...
    session = aterm_store.update_session(session_id, **update_fields)
    if not session:
...
@router.post("/api/aterm/sessions/{session_id}/reset", response_model=ATermSessionResponse)
@limiter.limit("10/minute")
async def reset_session(request: Request, session_id: str) -> ATermSessionResponse:
    """Reset a aterm session.
    ...
    session = aterm_store.get_session(new_session_id)
    if not session:
