...
This module provides data access for per-project aterm settings.
Each project can be enabled/disabled for aterm access and tracks
...
def _build_upsert_query(
    ...
    return psycopg.sql.SQL("""
        INSERT INTO aterm_project_settings
...
def prune_missing_projects(valid_project_ids: set[str]) -> int:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            DELETE FROM aterm_project_settings
