...
def _prepare_pane_slot(cur: Any, pane_order: int | None = None) -> int:
    ...
    cur.execute("LOCK TABLE aterm_panes IN SHARE ROW EXCLUSIVE MODE")
    cur.execute("SELECT COUNT(*) AS cnt FROM aterm_panes WHERE is_detached = false")
    row = cur.fetchone()
    ...
    if pane_order is not None:
        return pane_order
    cur.execute(
        "SELECT COALESCE(MAX(pane_order), -1) + 1 AS next_order FROM aterm_panes WHERE is_detached = false"
    )
    row = cur.fetchone()
