...
# ---------------------------------------------------------------------------
ATERM_SESSION_FIELDS = """id, name, user_id, project_id, working_dir, display_order,
               mode, session_number, is_alive, created_at, last_accessed_at,
               last_claude_session, claude_state, pane_id"""

_QUALIFIED_FIELDS = """aterm_sessions.id, aterm_sessions.name,
               aterm_sessions.user_id, aterm_sessions.project_id,
               aterm_sessions.working_dir, aterm_sessions.display_order,
               aterm_sessions.mode, aterm_sessions.session_number,
               aterm_sessions.is_alive, aterm_sessions.created_at,
               aterm_sessions.last_accessed_at, aterm_sessions.last_claude_session,
               aterm_sessions.claude_state, aterm_sessions.pane_id"""
...
def normalize_session_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    All session queries should SELECT ATERM_SESSION_FIELDS and pass results through this.
    """
    row["id"] = str(row["id"])
...
def list_sessions(include_dead: bool = False, include_detached: bool = True) -> list[dict[str, Any]]:
    """List aterm sessions, optionally filtering dead/detached."""
    conditions: list[str] = []
    if not include_dead:
        conditions.append("aterm_sessions.is_alive = true")
    if not include_detached:
        conditions.append("COALESCE(aterm_panes.is_detached, false) = false")

    where = f"WHERE {' AND '.join(conditions)}" if conditions else ""
    query = f"""
        SELECT {_QUALIFIED_FIELDS}
        FROM aterm_sessions
        LEFT JOIN aterm_panes ON aterm_panes.id = aterm_sessions.pane_id
        {where}
        ORDER BY aterm_sessions.display_order, aterm_sessions.created_at
    """
...
def get_session(session_id: SessionId) -> dict[str, Any] | None:
    """Get a session by ID."""
    query = f"SELECT {ATERM_SESSION_FIELDS} FROM aterm_sessions WHERE id = %s"
...
def create_session(
    ...
    """Create a new aterm session. Returns server-generated UUID."""
    with get_connection() as conn, conn.cursor() as cur:
        if project_id:
            cur.execute(
                """
                INSERT INTO aterm_sessions
    ...
    if not row:
        raise ValueError("Failed to create aterm session")
    return str(row[0])
...
def update_session(session_id: SessionId, **fields: Any) -> dict[str, Any] | None:
    ...
    query = psycopg.sql.SQL(
        f"""
        UPDATE aterm_sessions
        SET {{}}
        WHERE id = %s
        RETURNING {ATERM_SESSION_FIELDS}
    """
...
def delete_session(session_id: SessionId) -> bool:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            "DELETE FROM aterm_sessions WHERE id = %s RETURNING id",
            (_to_str(session_id),),
        )
        result = cur.fetchone()
        conn.commit()
...
def purge_dead_sessions(
    ...
    with get_connection() as conn, conn.cursor() as cur:
        if excluded:
            ...
            cur.execute(
                "DELETE FROM aterm_sessions WHERE is_alive = false AND last_accessed_at < %s",
                (cutoff,),
            )
...
def touch_session(session_id: SessionId) -> dict[str, Any] | None:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"""
            UPDATE aterm_sessions SET last_accessed_at = NOW()
            WHERE id = %s RETURNING {ATERM_SESSION_FIELDS}
            """,
            (_to_str(session_id),),
        )
        row = cur.fetchone()
...
def update_claude_session(session_id: SessionId, claude_session: str | None) -> None:
    """Update the last active Claude session for a aterm."""
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            "UPDATE aterm_sessions SET last_claude_session = NULLIF(%s, '') WHERE id = %s",
...
def update_claude_state(
    ...
    with get_connection() as conn, conn.cursor() as cur:
        if expected_state is not None:
            cur.execute(
                "UPDATE aterm_sessions SET claude_state = %s WHERE id = %s AND claude_state = %s RETURNING id",
                (state, _to_str(session_id), expected_state),
            )
        else:
            cur.execute(
                "UPDATE aterm_sessions SET claude_state = %s WHERE id = %s RETURNING id",
...
def get_claude_state(session_id: SessionId) -> str | None:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT claude_state FROM aterm_sessions WHERE id = %s",
            (_to_str(session_id),),
        )
        row = cur.fetchone()
