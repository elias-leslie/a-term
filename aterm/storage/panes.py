...
from .sessions import create_session as create_aterm_session
from .sessions import delete_session as delete_aterm_session
from .sessions import get_session as get_aterm_session
...
def _create_tmux_backed_session(
    ...
    session_id = create_aterm_session(
    ...
    try:
        ...
        delete_aterm_session(session_id)
        raise
    session = get_aterm_session(session_id)
    if not session:
...
def list_panes(include_detached: bool = False) -> list[dict[str, Any]]:
    ...
    query = (
        f"SELECT {PANE_FIELDS} FROM aterm_panes ORDER BY is_detached, pane_order, created_at"
        if include_detached
        else f"SELECT {PANE_FIELDS} FROM aterm_panes WHERE is_detached = false ORDER BY pane_order, created_at"
    )
...
def get_pane(pane_id: PaneId) -> dict[str, Any] | None:
    """Get a pane by ID."""
    return cast(dict[str, Any] | None, _execute_query(f"SELECT {PANE_FIELDS} FROM aterm_panes WHERE id = %s", (str(pane_id),)))
...
def create_pane_with_sessions(
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            f"INSERT INTO aterm_panes (pane_type, project_id, pane_order, pane_name, active_mode) VALUES (%s, %s, %s, %s, %s) RETURNING {PANE_FIELDS}",
...
def update_pane(pane_id: PaneId, **fields: Any) -> dict[str, Any] | None:
    ...
    query = psycopg.sql.SQL(
        f"UPDATE aterm_panes SET {{}} WHERE id = %s RETURNING {PANE_FIELDS}"
...
def delete_pane(pane_id: PaneId) -> bool:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("DELETE FROM aterm_sessions WHERE pane_id = %s", (nid,))
        cur.execute("DELETE FROM aterm_panes WHERE id = %s RETURNING id", (nid,))
        result = cur.fetchone()
        conn.commit()
...
def attach_pane(pane_id: PaneId) -> dict[str, Any] | None:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            f"UPDATE aterm_panes SET is_detached = false, pane_order = %s WHERE id = %s RETURNING {PANE_FIELDS}",
            (order, nid),
        )
        row = cur.fetchone()
        conn.commit()
