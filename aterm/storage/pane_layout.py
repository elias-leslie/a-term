...
def update_pane_order(pane_orders: list[tuple[str, int]]) -> None:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        for pid, order in pane_orders:
            cur.execute("UPDATE aterm_panes SET pane_order = %s WHERE id = %s", (order, pid))
        conn.commit()
...
def swap_pane_positions(pane_id_a: PaneId, pane_id_b: PaneId) -> bool:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute("SELECT id, pane_order FROM aterm_panes WHERE id IN (%s, %s)", (a, b))
        rows = cur.fetchall()
        if len(rows) != 2:
            return False
        orders = {str(row["id"]): row["pane_order"] for row in rows}
        cur.execute("UPDATE aterm_panes SET pane_order = %s WHERE id = %s", (orders[b], a))
        cur.execute("UPDATE aterm_panes SET pane_order = %s WHERE id = %s", (orders[a], b))
        conn.commit()
    return True
...
def count_panes(include_detached: bool = False) -> int:
    ...
    sql = "SELECT COUNT(*) FROM aterm_panes" + ("" if include_detached else " WHERE is_detached = false")
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(sql)
        row = cur.fetchone()
