"""CLI for the shared local Terminal session broker."""
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="tsession", description="Terminal session broker")
    subparsers = parser.add_subparsers(dest="command", required=True)

    open_parser = subparsers.add_parser("open", help="Reuse or create a Terminal-managed tool session")
    ...
    list_parser = subparsers.add_parser("list", help="List Terminal-managed project sessions")
...
def _run_list(args: argparse.Namespace) -> int:
    ...
    if not targets:
        print("No Terminal-managed sessions found.")
        return 0
