...
def _pkg() -> object:
    """Return the terminal.utils.tmux package module (avoids circular import)."""
    return sys.modules["terminal.utils.tmux"]
...
def _run_tmux_new_session(args: list[str], session_name: str) -> tuple[bool, str]:
    ...
    cmd = [
        ...
        f"--description=terminal tmux session spawn {session_name}",
        "tmux",
        *args,
    ]
    try:
