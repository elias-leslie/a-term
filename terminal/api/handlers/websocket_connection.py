...
async def _wait_for_initial_resize(
    ...
    """Wait for initial resize event from frontend (syncs terminal dimensions)."""
    try:
...
def _make_output_flush_callback(
    ...
    async def on_flush(batch: str) -> None:
        terminal_metrics.inc("flush_count")  # type: ignore[union-attr]
        terminal_metrics.inc("total_bytes_flushed", len(batch.encode("utf-8")))  # type: ignore[union-attr]
        terminal_metrics.inc("messages_sent")  # type: ignore[union-attr]
...
async def _teardown_session_resources(
    ...
    terminal_metrics.dec("active_connections")  # type: ignore[union-attr]
    terminal_metrics.dec("active_sessions")  # type: ignore[union-attr]
...
async def _run_session(
    ...
    terminal_metrics = get_metrics()
    terminal_metrics.inc("active_connections")
    terminal_metrics.inc("active_sessions")
    terminal_metrics.inc("total_sessions_created")
    ...
    on_flush = _make_output_flush_callback(terminal_metrics, scrollback_tracker, recorder)
    ...
    try:
        ...
        await _teardown_session_resources(
            ...
            scrollback_sync, recorder, diag_registry, terminal_metrics,
        )
