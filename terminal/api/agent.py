...
Provides:
- Get agent state for a terminal session (state machine based)
- Start an agent in a terminal session with state machine
...
class AgentStateResponse(BaseModel):
    """Agent state for a terminal session (state machine based)."""
    session_id: str
...
@router.get(
    "/api/terminal/sessions/{session_id}/agent-state",
...
async def get_claude_state_endpoint(session_id: str) -> AgentStateResponse:
    """Legacy alias: Get agent state for a terminal session."""
    return await get_agent_state_endpoint(session_id)


@router.post(
    "/api/terminal/sessions/{session_id}/start-agent",
...
async def start_agent(session_id: str, background_tasks: BackgroundTasks) -> StartAgentResponse:
    """Start an agent in a terminal session using state-machine guards.
...
async def start_claude(session_id: str, background_tasks: BackgroundTasks) -> StartAgentResponse:
    """Legacy alias: Start agent in a terminal session."""
