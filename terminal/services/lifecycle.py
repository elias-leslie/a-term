...
def create_session(
    ...
    """Create a new terminal session atomically (DB + tmux, rollback on failure).
...
def delete_session(session_id: str) -> bool:
    """Delete a terminal session atomically. Idempotent."""
...
def reset_project_sessions(
    ...
    all_sessions = terminal_store.get_all_project_sessions(project_id)
...
def reset_all_sessions() -> int:
    """Reset all terminal sessions globally. Returns count reset."""
...
def disable_project_terminal(project_id: str) -> bool:
    ...
    logger.info("project_terminal_disabled", project_id=project_id, deleted_sessions=len(all_sessions))
    return True
