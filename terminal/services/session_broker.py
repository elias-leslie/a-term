"""Shared local session broker for Terminal-aware tool launchers."""
...
class BrokerSessionTarget:
    """A Terminal-managed session target that local wrappers can attach to."""
...
def _pane_name_for_project(project_id: str, panes: list[dict[str, Any]]) -> str:
    ...
    base_name = project_id[:1].upper() + project_id[1:]
...
def ensure_project_tool_session(
    ...
    """Reuse or create the Terminal-managed project session for a tool."""
...
def list_project_tool_sessions(tool_slug: str | None = None) -> list[BrokerSessionTarget]:
    """List Terminal-managed project tool sessions that wrappers can attach to."""
