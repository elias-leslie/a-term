...
from .sessions import TERMINAL_SESSION_FIELDS, normalize_session_row
...
def fetch_sessions_for_pane(pane_id: PaneId) -> list[dict[str, Any]]:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"SELECT {TERMINAL_SESSION_FIELDS} FROM terminal_sessions WHERE pane_id = %s ORDER BY mode",
            (str(pane_id),),
        )
...
def list_panes_with_sessions_data(pane_ids: list[str]) -> dict[str, list[dict[str, Any]]]:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"SELECT {TERMINAL_SESSION_FIELDS} FROM terminal_sessions WHERE pane_id IS NOT NULL ORDER BY pane_id, mode"
        )
