"""Terminal sessions storage layer.
...
_QUALIFIED_FIELDS = """terminal_sessions.id, terminal_sessions.name,
               terminal_sessions.user_id, terminal_sessions.project_id,
               terminal_sessions.working_dir, terminal_sessions.display_order,
               terminal_sessions.mode, terminal_sessions.session_number,
               terminal_sessions.is_alive, terminal_sessions.created_at,
               terminal_sessions.last_accessed_at, terminal_sessions.last_claude_session,
               terminal_sessions.claude_state, terminal_sessions.pane_id"""
...
def normalize_session_row(row: dict[str, Any]) -> dict[str, Any]:
    ...
    All session queries should SELECT TERMINAL_SESSION_FIELDS and pass results through this.
    """
    row["id"] = str(row["id"])
...
def list_sessions(include_dead: bool = False, include_detached: bool = True) -> list[dict[str, Any]]:
    """List terminal sessions, optionally filtering dead/detached."""
    conditions: list[str] = []
    if not include_dead:
        conditions.append("terminal_sessions.is_alive = true")
    if not include_detached:
        conditions.append("COALESCE(terminal_panes.is_detached, false) = false")

    where = f"WHERE {' AND '.join(conditions)}" if conditions else ""
    query = f"""
        SELECT {_QUALIFIED_FIELDS}
        FROM terminal_sessions
        LEFT JOIN terminal_panes ON terminal_panes.id = terminal_sessions.pane_id
        {where}
        ORDER BY terminal_sessions.display_order, terminal_sessions.created_at
    """
...
def get_session(session_id: SessionId) -> dict[str, Any] | None:
    """Get a session by ID."""
    query = f"SELECT {TERMINAL_SESSION_FIELDS} FROM terminal_sessions WHERE id = %s"
...
def create_session(
    ...
    """Create a new terminal session. Returns server-generated UUID."""
...
def purge_dead_sessions(
    ...
    with get_connection() as conn, conn.cursor() as cur:
        if excluded:
            ...
            cur.execute(
                "DELETE FROM terminal_sessions WHERE is_alive = false AND last_accessed_at < %s",
                (cutoff,),
            )
...
def touch_session(session_id: SessionId) -> dict[str, Any] | None:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"""
            UPDATE terminal_sessions SET last_accessed_at = NOW()
            WHERE id = %s RETURNING {TERMINAL_SESSION_FIELDS}
            """,
            (_to_str(session_id),),
        )
        row = cur.fetchone()
...
def update_claude_session(session_id: SessionId, claude_session: str | None) -> None:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            "UPDATE terminal_sessions SET last_claude_session = NULLIF(%s, '') WHERE id = %s",
...
def update_claude_state(
    ...
    with get_connection() as conn, conn.cursor() as cur:
        if expected_state is not None:
            cur.execute(
                "UPDATE terminal_sessions SET claude_state = %s WHERE id = %s AND claude_state = %s RETURNING id",
                (state, _to_str(session_id), expected_state),
            )
        else:
            cur.execute(
                "UPDATE terminal_sessions SET claude_state = %s WHERE id = %s RETURNING id",
...
def get_claude_state(session_id: SessionId) -> str | None:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            "SELECT claude_state FROM terminal_sessions WHERE id = %s",
            (_to_str(session_id),),
        )
        row = cur.fetchone()
