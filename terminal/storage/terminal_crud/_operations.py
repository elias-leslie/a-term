"""Terminal session CRUD operations: list, get, create, update, delete."""
...
from ._helpers import TERMINAL_SESSION_FIELDS, _execute_session_query, _row_to_dict
...
def _insert_session_with_project(
    cur: Any,
    name: str,
    user_id: str | None,
    project_id: str,
    ...
    """Insert a session with computed session_number for a specific project."""
    cur.execute(
        """
...
def _insert_session_without_project(
    cur: Any,
    name: str,
    user_id: str | None,
    working_dir: str | None,
    ...
    """Insert a session without a project, using session_number=1."""
    cur.execute(
        """
        INSERT INTO terminal_sessions
            (name, user_id, project_id, working_dir, mode, session_number, pane_id)
...
def create_session(
    ...
    with get_connection() as conn, conn.cursor() as cur:
        if project_id:
            _insert_session_with_project(cur, name, user_id, project_id, working_dir, mode, pane_id)
        else:
            _insert_session_without_project(cur, name, user_id, working_dir, mode, pane_id)
        row = cur.fetchone()
        conn.commit()
