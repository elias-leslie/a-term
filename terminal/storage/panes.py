"""Terminal panes storage layer — CRUD and session-creation.
...
from .sessions import create_session as create_terminal_session
from .sessions import delete_session as delete_terminal_session
from .sessions import get_session as get_terminal_session
...
def _create_tmux_backed_session(
    ...
    try:
        ...
        delete_terminal_session(session_id)
        raise
...
def list_panes(include_detached: bool = False) -> list[dict[str, Any]]:
    ...
    query = (
        f"SELECT {PANE_FIELDS} FROM terminal_panes ORDER BY is_detached, pane_order, created_at"
        if include_detached
        else f"SELECT {PANE_FIELDS} FROM terminal_panes WHERE is_detached = false ORDER BY pane_order, created_at"
    )
...
def get_pane(pane_id: PaneId) -> dict[str, Any] | None:
    """Get a pane by ID."""
    return cast(dict[str, Any] | None, _execute_query(f"SELECT {PANE_FIELDS} FROM terminal_panes WHERE id = %s", (str(pane_id),)))
...
def delete_pane(pane_id: PaneId) -> bool:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute("DELETE FROM terminal_sessions WHERE pane_id = %s", (nid,))
        cur.execute("DELETE FROM terminal_panes WHERE id = %s RETURNING id", (nid,))
        result = cur.fetchone()
        conn.commit()
...
def attach_pane(pane_id: PaneId) -> dict[str, Any] | None:
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        ...
        cur.execute(
            f"UPDATE terminal_panes SET is_detached = false, pane_order = %s WHERE id = %s RETURNING {PANE_FIELDS}",
            (order, nid),
        )
        row = cur.fetchone()
        conn.commit()
