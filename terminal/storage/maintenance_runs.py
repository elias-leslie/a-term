"""Persistence helpers for terminal maintenance run history."""
...
_FIELD_NAMES = [field.strip() for field in MAINTENANCE_RUN_FIELDS.split(",")]
...
def create_run(reason: str) -> str:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO terminal_maintenance_runs (reason, status)
            VALUES (%s, 'running')
            RETURNING id
            """,
            (reason,),
...
def complete_run(
    ...
    *,
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            f"""
            UPDATE terminal_maintenance_runs
            SET status = %s,
                completed_at = NOW(),
                duration_ms = %s,
...
def list_recent_runs(limit: int = 10) -> list[dict[str, Any]]:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            FROM terminal_maintenance_runs
