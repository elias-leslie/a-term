"""A-Term Files API - file upload endpoint.

Stores uploaded files server-side; returns path for use in a_term commands.
"""
...
def _save_upload(content: bytes, content_type: str) -> tuple[str, str]:
    ...
    Returns a tilde-prefixed path (e.g., ~/a-term-uploads/abc.png) to avoid
...
@router.post("/api/a-term/files", response_model=FileUploadResponse)
@limiter.limit("10/minute")
async def upload_file(request: Request, file: UploadFile) -> FileUploadResponse:
    """Upload a file for use in a_term commands.

    Stores in the configured upload directory with UUID naming.
