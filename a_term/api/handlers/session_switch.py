...
def _track_session_switch(a_term_session_id: str, to_session: str) -> dict[str, Any]:
    """Update stored session target for the given a_term session.
    ...
    if to_session.startswith(BASE_SESSION_PREFIX):
        logger.info(LOG_SWITCH_TO_BASE, a_term=a_term_session_id)
        a_term_store.update_claude_session(a_term_session_id, None)
        return {"status": STATUS_CLEARED}

    logger.info(LOG_SWITCH_DETECTED, a_term=a_term_session_id, target=to_session)
    a_term_store.update_claude_session(a_term_session_id, to_session)
...
def _extract_a_term_session_id(
    from_session: str,
) -> tuple[str, JSONResponse | None]:
    """Extract and validate the a_term session UUID from the base session name.
    ...
    """
    a_term_session_id = from_session[len(BASE_SESSION_PREFIX):]
    try:
        uuid.UUID(a_term_session_id)
    except ValueError:
        logger.warning(LOG_INVALID_SESSION_ID, raw=a_term_session_id)
        return "", JSONResponse(
            status_code=400,
            content={"error": ERROR_INVALID_SESSION_ID},
        )
    return a_term_session_id, None
...
def handle_session_switch(
    ...
    # Only track switches FROM a a_term base session
    ...
    a_term_session_id, error = _extract_a_term_session_id(from_session)
    if error is not None:
        return error

    return _track_session_switch(a_term_session_id, to_session)
