...
Provides:
- Get agent state for a a_term session (state machine based)
- Start an agent in a a_term session with state machine
...
@router.get(
    "/api/a-term/sessions/{session_id}/agent-state",
...
async def get_claude_state_endpoint(session_id: str) -> AgentStateResponse:
    """Legacy alias: Get agent state for a a_term session."""
    return await get_agent_state_endpoint(session_id)


@router.post(
    "/api/a-term/sessions/{session_id}/start-agent",
...
async def start_agent(session_id: str, background_tasks: BackgroundTasks) -> StartAgentResponse:
    """Start an agent in a a_term session using state-machine guards.
    ...
    if await is_agent_running(tmux_session, process_name):
        a_term_store.update_claude_state(session_id, "running", expected_state=current_state)
...
async def start_claude(session_id: str, background_tasks: BackgroundTasks) -> StartAgentResponse:
    """Legacy alias: Start agent in a a_term session."""
