"""Persistence helpers for a_term maintenance run history."""
...
def create_run(reason: str) -> str:
    ...
    with get_connection() as conn, conn.cursor() as cur:
        cur.execute(
            """
            INSERT INTO a_term_maintenance_runs (reason, status)
            VALUES (%s, 'running')
            RETURNING id
            """,
            (reason,),
...
def complete_run(
    ...
    *,
    ...
    with get_connection() as conn, conn.cursor(row_factory=dict_row) as cur:
        cur.execute(
            f"""
            UPDATE a_term_maintenance_runs
            SET status = %s,
                completed_at = NOW(),
                duration_ms = %s,
