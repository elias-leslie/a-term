...
async def background_verify_agent_start(
    ...
    try:
        ...
        if await is_agent_running(tmux_session, process_name):
            updated = a_term_store.update_claude_state(
            ...
            updated = a_term_store.update_claude_state(session_id, "error", expected_state="starting")
            if updated:
        ...
        with contextlib.suppress(Exception):
            a_term_store.update_claude_state(session_id, "error", expected_state="starting")
...
def atomically_set_starting(session_id: str, current_state: AgentState) -> AgentState | None:
    ...
    if not updated:
        return cast(AgentState, a_term_store.get_claude_state(session_id) or "not_started")
    return None
...
def ensure_agent_running_sync(session_id: str) -> bool:
    ...
    if not lifecycle.ensure_session_alive(session_id):
        a_term_store.update_claude_state(session_id, "error")
    ...
    if _is_agent_running_in_session_sync(tmux_session, process_name):
        a_term_store.update_claude_state(session_id, "running")
        return False

    current_state = cast(AgentState, session.get("claude_state") or "not_started")
    if current_state == "running":
        a_term_store.update_claude_state(session_id, "not_started")
    ...
    if _is_agent_running_in_session_sync(tmux_session, process_name):
        a_term_store.update_claude_state(session_id, "running", expected_state="starting")
        return True
