...
from terminal.utils.tmux.sessions import (
...
def test_create_tmux_session_uses_systemd_scope_when_available() -> None:
    ...
    with (
        patch("terminal.utils.tmux.tmux_session_exists", return_value=False),
        patch("terminal.utils.tmux._apply_session_options") as mock_apply,
        patch(
            "terminal.utils.tmux.sessions._recreate_initial_window_with_session_history_limit"
        ) as mock_recreate,
        patch("terminal.utils.tmux._can_spawn_tmux_scope", return_value=True),
        patch(
            "terminal.utils.tmux.subprocess.run",
            return_value=MagicMock(returncode=0, stdout="", stderr=""),
        ) as mock_run,
        patch("terminal.utils.tmux._uuid_mod.uuid4", return_value=scope_id),
    ):
...
def test_create_tmux_session_falls_back_without_user_scope_support() -> None:
    with (
        ...
        patch("terminal.utils.tmux._can_spawn_tmux_scope", return_value=False),
