"""add aterm maintenance runs table
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS aterm_maintenance_runs (
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_aterm_maintenance_runs_started_at
        ON aterm_maintenance_runs(started_at DESC);
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_aterm_maintenance_runs_status_started_at
        ON aterm_maintenance_runs(status, started_at DESC);
        """
    )


def downgrade() -> None:
    """Drop aterm maintenance run history."""
    op.execute("DROP INDEX IF EXISTS idx_aterm_maintenance_runs_status_started_at;")
    op.execute("DROP INDEX IF EXISTS idx_aterm_maintenance_runs_started_at;")
    op.execute("DROP TABLE IF EXISTS aterm_maintenance_runs;")
