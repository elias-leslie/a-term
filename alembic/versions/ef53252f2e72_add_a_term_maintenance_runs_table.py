"""add a_term maintenance runs table
...
down_revision: str | Sequence[str] | None = "f3b12a7c9d4e"
...
def upgrade() -> None:
    ...
    op.execute(
        """
        CREATE TABLE IF NOT EXISTS a_term_maintenance_runs (
    ...
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_a_term_maintenance_runs_started_at
        ON a_term_maintenance_runs(started_at DESC);
        """
    )
    op.execute(
        """
        CREATE INDEX IF NOT EXISTS idx_a_term_maintenance_runs_status_started_at
        ON a_term_maintenance_runs(status, started_at DESC);
        """
    )


def downgrade() -> None:
    """Drop a_term maintenance run history."""
    op.execute("DROP INDEX IF EXISTS idx_a_term_maintenance_runs_status_started_at;")
    op.execute("DROP INDEX IF EXISTS idx_a_term_maintenance_runs_started_at;")
    op.execute("DROP TABLE IF EXISTS a_term_maintenance_runs;")
